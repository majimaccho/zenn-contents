...
#### 影響範囲を調べるコスト
カラムを別テーブルに移動させる際に、アプリケーションの影響範囲を調査する必要があります。この時、カラムに依存していなくても、テーブルに依存が増えていると、本当にカラムに依存がないかどうかを確認する必要が出てきます。時間が経つほど、テーブルに対する依存が増えるため、カラムを別テーブルに移動させるコストが増大します。
