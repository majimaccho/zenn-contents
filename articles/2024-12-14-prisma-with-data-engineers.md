...
<!--
この部分の表現の意図が気になりました。
PostgreSQLは `""` で囲むことで大文字と小文字を混在させたり、非ASCII文字をテーブル名やカラム名に使うことができます。
MySQLやBigQueryでは `` で囲みます（もしかしたら、こちらの記法の方が一般的なSQLテキストなどでいるかもしれません）
この辺枝葉末節なので、「適切にエスケープをすることで」くらいでいいかもしれません

また、MySQLでは https://dev.mysql.com/doc/refman/8.0/ja/identifier-case-sensitivity.html
...
ということなので、「ローカルでは動いているのだが……」とかその逆とかが起きえる気がします。
-->
